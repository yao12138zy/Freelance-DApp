...
async function connectWallet() {
  ...
  try {
    ...
    if (chainId !== appConfig.chainIdHex) {
      alert(
        `Wrong network. Please switch MetaMask to chain ID ${appConfig.chainIdHex} (Ganache).`,
      );
      return false;
    }
    ...
    marketplaceContract = new web3Instance.eth.Contract(
      appConfig.abi,
      appConfig.contractAddress,
    );
    if (appConfig.arbitrationAddress && appConfig.arbitrationAbi) {
      arbitrationContract = new web3Instance.eth.Contract(
        appConfig.arbitrationAbi,
        appConfig.arbitrationAddress,
      );
    }
    ...
    // If the page has already loaded job details, refresh them so buttons appear.
